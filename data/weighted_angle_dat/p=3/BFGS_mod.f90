...
PARAMETER (NMAX=50,ITMAX=200,STPMX=10.d0,EPS=3.d-10,TOLX=4.d0*EPS) 
EXTERNAL dfunc,func
...
!WHERE P IS THE INITIAL SET OF PARAMETERS?
fp=func(n,p)
call dfunc(n,p,g,func)
sum=0.d0
grad_zero=.true.
do i = 1,NMAX
	if (dabs(g(i)) .gt. 1.d-12) then
		grad_zero=.false.
	endif
enddo
if (grad_zero) then
...
do its=1,ITMAX  
	...
	do i=1,n 
		...
		p(i)=pnew(i)
	enddo
	!PRINT HOW IT IS GOING: ITERATION, RESULT, PARAMETERS 
	!print *, 'it, fp, pi', its, fp, (p(i),i=1,n)
	test=0.d0
...
if (slope .ge. 0.d0) then
	print *, 'roundoff problem in lnsrch, slope:',slope
	stop
endif
!Compute λmin.
test=0.d0 
