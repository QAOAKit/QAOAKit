...
if not Path(
    build_table_graph2pynauty_folder, "../data/qaoa-dataset-version1/"
).is_dir():
    ...
    # remove top folder
    folder = list(
        Path(build_table_graph2pynauty_folder, "../data/").glob("QAOAKit-data-*")
    )
    assert len(folder) == 1
    folder = folder[0]
...
for n_qubits in range(3, 10):
    ...
    with open(
        Path(
            build_table_graph2pynauty_folder,
