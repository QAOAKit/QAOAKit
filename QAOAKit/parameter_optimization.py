...
def train_kde(p, n, n_jobs=1, bandwidth_range=np.logspace(-2, 1, 20)):
    ...
    if p == 1:
        data = df.apply(lambda x: np.hstack([np.array(x['gamma']) / np.arctan(1/np.sqrt(x['average degree']-1)), np.array(x['beta'])]), axis=1).values
    else:
        data = df.apply(lambda x: np.hstack([np.array(x['gamma']) * np.sqrt(x['average degree']), np.array(x['beta'])]), axis=1).values
    data = np.stack(data)
