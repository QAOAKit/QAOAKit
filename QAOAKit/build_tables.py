...
def build_graph2angles():
    ...
    pickle.dump(tables, open(Path(build_tables_folder, f"../data/lookup_tables/graph2angles.p"), "wb"))
    del tables
...
def build_graph2pynauty():
    ...
    for n_qubits in tqdm(range(3,10)):
        ...
        with open(Path(build_tables_folder, "../data/qaoa-dataset-version1/Graphs/graph"+str(n_qubits)+"c.txt")) as f:
    ...
    pickle.dump(tables, open(Path(build_tables_folder, f"../data/lookup_tables/graph2pynauty.p"), "wb"))
    del tables
...
def build_graph2pynauty_large():
    ...
    for n_qubits in tqdm(range(3,10)):
        ...
        pickle.dump(table, open(Path(build_tables_folder, f"../data/lookup_tables/graph2pynauty_large_{n_qubits}.p"), "wb"))
        del table
        gc.collect()
...
def build_full_qaoa_dataset():
    ...
    for n_qubits in tqdm(range(3,10)):
        graph_table = pickle.load(open(Path(build_tables_folder, f"../data/lookup_tables/graph2pynauty_large_{n_qubits}.p"), "rb"))
    ...
    df.to_pickle(Path(build_tables_folder, "../data/lookup_tables/full_qaoa_dataset_table.p"))
    del df
    print(f"Done building full_qaoa_dataset table.")

def build_3_reg_dataset():
    print("Building  3-regular graph table")
    with open(Path(build_tables_folder, "../data/3_regular/3r_WURTZ_ensemble.json")) as json_file:
    ...
    df.to_pickle(Path(build_tables_folder, "../data/lookup_tables/3_reg_dataset_table.p"))
